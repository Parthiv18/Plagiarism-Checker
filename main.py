...
def consecutiveCounter(newList):
    ...
    for i in range(len(newList) - 3): #loop through the list
        ...
        if countsOfFour == 1: #if the counts of 4 is one make new counter to 1
            count2 = 1
            #print(countsOfFour)
        elif (countsOfFour % 2 == 0) and countsOfFour > 1: #Pattern [3 5 ...] use arthemetic series to solve for n
            count2 = ((countsOfFour - 3 + 2) / 2) - 0.5
        elif (countsOfFour % 2 != 0) and countsOfFour > 1: #Pattern [3 5 ...] use arthemetic series to solve for n
            count2 = (countsOfFour - 3 + 2) / 2
    #print(count2)
...
if(checkFile(readFile1,readFile2)==True): #Rule 1
    print("Detected 100%")
elif (consecutiveCounter(newList) > 0.0):
    print("Detected: " + str(consecutiveCounter(newList)) + "%") #Rule 2
